...
describe("buildBikingEnsemble", () => {
  it("does not recommend leg-only mid-layers for torso zone", () => {
    ...
    const ensemble = buildBikingEnsemble(
      categorized,
  ...
  it("selects both torso and legs mid-layers independently", () => {
    const torsoBase = createGarment({
      id: "torso-base",
      category: "base_layer",
      covers_torso: true,
      garment_thermal_properties: { rcl_whole_body: 0.1, evap_potential: 0.35 },
    });
    const legsBase = createGarment({
      id: "legs-base",
      category: "base_layer",
    ...
    const torsoMid = createGarment({
      ...
      category: "mid_layer_light",
      covers_torso: true,
      garment_thermal_properties: { rcl_whole_body: 0.3, evap_potential: 0.3 },
    });
    const legsMid = createGarment({
      id: "legs-tights",
      category: "mid_layer_light",
      covers_legs: true,
      garment_thermal_properties: { rcl_whole_body: 0.2, evap_potential: 0.3 },
    ...
    const ids = ensemble.map((g) => g.id);
    expect(ids).toContain("torso-fleece");
    expect(ids).toContain("legs-tights");
  });
